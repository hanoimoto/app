...
(function(){
  ...
  /* === CSS NÂNG CẤP (từ gợi ý của bạn) === */
  #mta-card{
    position:fixed;
    left:16px;
    /* dính theo safe area giống bubble */
    bottom: calc(16px + env(safe-area-inset-bottom, 0));
    width:min(420px, calc(100% - 24px));

    /* cho nó tự cao, nhưng không vượt quá màn hình */
    height:auto;
  ...
  @media(max-width:520px){
    /* Đã bị ghi đè bởi CSS nâng cấp ở trên, nhưng giữ lại để tương thích */
    #mta-card{right:8px;left:8px;width:auto;}
  }
  ...
  // === NÂNG CẤP 3: fallbackAnswer hiểu 'mode' ===
  function fallbackAnswer(q, lang, profile, mode){
    // mode: "rent" | "buy" | "both" | "unknown"
    const isBuy  = mode === "buy";
    const isRent = mode === "rent" || mode === "both";

    if (lang === "vi"){
      // Khách nghiêng về MUA xe
      if (isBuy){
      ...
      // Local, không rõ thuê/mua -> giải thích cả hai
      if (profile === "local"){
  ...
  // === NÂNG CẤP 2: buildAnswer dùng 'mode' ===
  function buildAnswer(q){
    ...
    const mode    = detectMode(text); // thuê hay mua

    // Nếu khách rõ ràng hỏi về MUA xe -> ưu tiên nội dung mua, không spam bảng giá thuê theo ngày
  ...
  // === NÂNG CẤP 4: setTagsFor hiểu 'mode' ===
  // === SỬA LỖI (theo phân tích): Đổi tên 'mode' thành 'viewMode' để rõ nghĩa ===
  function setTagsFor(meta){
    const lang = (meta && meta.lang) || "en";
    const viewMode = meta.mode || "answer"; // <-- ĐÃ SỬA
    const intents = meta.intents || {};

    if (viewMode === "welcome"){ // <-- ĐÃ SỬA
      const html = lang === "vi"
        ? `
    ...
    // Generic – nếu user nghiêng về mua xe thì đổi suggestion
    // (Chỗ này 'meta.mode' vẫn đúng, là 'rent'/'buy'/'unknown' từ buildAnswer)
  ...
  async function handleUserInput(raw){
    ...
    const meta = buildAnswer(q); // buildAnswer đã được nâng cấp
    let ans = meta.answer || "";
    ...
    // (Nâng cấp) Truyền thêm mode và lastText để quick replies thông minh hơn
    setTagsFor({
      lang: meta.lang,
  ...
  function boot(){
    ...
    initEvents();
    if (CFG.debug){
      console.log("%cMotoAI v39 (Upgraded) — Left + EN/VI + Rent/Buy Logic active","color:"+CFG.themeColor+";font-weight:bold");
    }
  }
